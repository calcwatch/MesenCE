...
{
	return StringUtilities::EndsWith(StringUtilities::ToLower(filename), ".zst");
}
...
void TraceLogFileSaver::StopLogging()
{
	...
	if(_outputFile.is_open()) {
		bool success = !_loggingError;

		if(wasEnabled && success) {
			success = FlushOutputBuffer();

			if(success && _compressed) {
				success = FinalizeCompressedStream();
			}
		}

		_outputFile.close();
		success &= (bool)_outputFile;

		if(!success) {
			MessageManager::Log("[Trace Logger] Error writing trace log file.");
		}
	}
...
void TraceLogFileSaver::Log(const string& log)
{
	if(!_enabled) {
		return;
	}
